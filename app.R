...
ui <- fluidPage(
  ...
  navbarPage(
    ...
    tabPanel(
      ...
      # **New Tab: GENERIC TOOL**
      tabPanel(
        "GENERIC TOOL",
        sidebarLayout(
          sidebarPanel(
            selectInput("filetype", "Select file type:", choices = c("PNG", "PDF")),
            downloadButton("downloadPlot", "Download Plot"),  # <- DO NOT disable it in UI
            fileInput("file", "Upload Data File", accept = c(".csv", ".json", ".xls", ".xlsx")),
            selectInput("col_x", "Select X Column", choices = NULL),
            selectInput("col_y", "Select Y Column (Only for Scatterplot)", choices = NULL, selected = NULL),
            selectInput("plot_type", "Select Plot Type", choices = c("Histogram" = "hist", "Boxplot" = "boxplot", "Scatterplot" = "scatter")),
    ...
    $(document).on('shiny:connected', function() {
      $('#downloadPlot').attr('download', 'plot.png');  // Default to PNG
    });

    $(document).on('shiny:value', function(event) {
      if (event.name === 'downloadPlot') {
        var filetype = $('#filetype').val();
        $('#downloadPlot').attr('download', 'plot.' + filetype.toLowerCase());
      }
    });
  "))
      )
...
server <- function(input, output, session) {
  ...
  observeEvent(input$file, {
    ...
    if (file_ext == "csv") {
      ...
      data <- jsonlite::fromJSON(input$file$datapath, flatten = TRUE)
    ...
    data_reactive(data)
    updateSelectInput(session, "col_x", choices = names(data))
    updateSelectInput(session, "col_y", choices = names(data))
  })
  ...
  observeEvent(input$run, {
    req(data_reactive(), input$col_x)
    
    output$plot <- renderPlot({
      data <- data_reactive()
      
      if (input$plot_type == "hist") {
        hist(data[[input$col_x]], main = paste("Histogram of", input$col_x), col = "skyblue", border = "white")
      } else if (input$plot_type == "boxplot") {
        boxplot(data[[input$col_x]], main = paste("Boxplot of", input$col_x), col = "lightgreen")
      } else if (input$plot_type == "scatter") {
        req(input$col_y)
        plot(data[[input$col_x]], data[[input$col_y]], 
             main = paste("Scatterplot of", input$col_x, "vs", input$col_y), 
             col = "red", pch = 19)
      }
    })
    
    shinyjs::enable("downloadPlot")  # Enable download button when a plot is generated
  })
  ...
  output$downloadPlot <- downloadHandler(
    filename = function() {
      ...
      print(paste("Generating file: ", input$col_x, ext))  # Debugging line to check if downloadHandler is being triggered.
      paste0("plot-", input$col_x, "-", Sys.Date(), ".", ext)  # Ensure the correct file extension
    },
    
    content = function(file) {
      print("Inside content function")  # Another debugging line to check if content() is running
      req(input$col_x)
      
      ext <- tolower(input$filetype)
      print(paste("File type chosen:", ext))  # Check which filetype is selected.
      flush.console()  # Forces the print statement to appear immediately.
      ...
      dev.off()  # Finalize the plot and save the file
      print("Plot file saved!")  # Ensure it's saved
    }
  )
  ...
  output$summary_output <- renderTable({
    req(data_reactive(), input$col_x)
    summary(data_reactive()[[input$col_x]])
  })
  ...
  output$corrPlot <- renderPlot({
    ...
    output$downloadPlot <- downloadHandler(
      filename = function() {
        paste("plot-", input$var, "-", Sys.Date(), ".", tolower(input$filetype), sep = "")
      },
      content = function(file) {
        ...
        print(plotInput())  # Ensure the correct plot is used
        dev.off()
      }
    )
