                           ...
                           fluidRow(
                             column(12, class = "compact-container",
                                    ...
                                    conditionalPanel(
                                      ...
                                      radioButtons(inputId = "plot_choice_tabular4", label = "Choose Plot:", 
                                                   choices = list("Age" = "age4", 
                                                                  "Sex" = "sex4",
...
server <- function(input, output) {
  ...
  output$data_dsout4 <- renderDataTable({
    datatable(luad_data4, options = list(scrollX = TRUE, lengthMenu = list(c(10, 25, 50, -1), c('10', '25', '50', 'All'))))
  })
  ...
  output$data_summary4 <- renderPrint({
    summary(luad_data4)
  })
  ...
  total_samples_luad4 <- nrow(luad_data4)
  ...
  filtered_data <- reactive({
    if (input$plot_data_choice_tabular == "luad_data1") {
      ...
      switch(input$plot_choice_tabular4,
  ...
  output$dynamic_plot <- renderPlot({
    ...
    if (input$plot_data_choice_tabular == "luad_data1") {
      ...
      plot <- switch(input$plot_choice_tabular4,
                     "age4" = {
                       average_age <- mean(filt_data$Age, na.rm = TRUE)
                       ggplot(filt_data, aes(x = Age)) +
                         geom_bar() +
                         labs(title = "Distribution of Diagnosis Age", x = "Age", y = "Count") +
                         annotate("text", label = paste("Valid Filtered Samples:", total_filtered_samples, "NA:", (total_samples_luad3 - total_filtered_samples)), 
